...
if($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		...
		$sqlFeeCon			=	"SELECT SUM(CONCESSION_AMOUNT) AS TOT_CON FROM `fee_concession` WHERE ADMISSION_ID = '".$admId."' ";
		$exeSqlFeeCon		=	$mysqli->query($sqlFeeCon);
		$resExeSqlFeeCon	=	$exeSqlFeeCon->fetch_assoc();
		if($resExeSqlFeeCon['TOT_CON'] == NULL){
			$row['TOT_CON']	= 0;
		}	
		else {
			$row['TOT_CON']	=	$resExeSqlFeeCon['TOT_CON'];
		}
		...
		$query3				=	"SELECT T3.ADMISSION_ID,T3.RECEIPT_NO, T3.RECEIPT_ID, T3.RECEIPT_DATE, T5.feehead AS FEE_HEAD, T5.feetype AS FEE_TYPE, T4.Amount AS FEE_AMOUNT FROM fee_receipt T3 JOIN fee_transanction T4 JOIN feeheads T5 ON T5.feeheadId = T4.feeHead WHERE T3.RECEIPT_ID = T4.RECEIPT_ID AND T3.RECEIPT_ID IN(SELECT RECEIPT_ID FROM `fee_receipt` WHERE ADMISSION_ID = '".$admId."') AND T3.YEAR_ID = '".$yearId."' ORDER BY T4.RECEIPT_ID ASC ";
...
foreach( $arr as $pitem ) {
	...
	$parr	=	array('U_PDF' => $_SESSION['SESS_MEMBER_DBNAME'], 'ADMISSION_ID' => $pitem['ADMISSION_ID'], 'NAME' => $pitem['NAME'], 'FATHER_NAME' => $pitem['FATHER_NAME'], 'STD' => $pitem['STD'], 'SEC' => $pitem['SEC'],  'STD_SEC' => $pitem['STD_SEC'], 'TOT_AMOUNT' => $pitem['TOT_AMOUNT'], 'TOT_PAID' => $pitem['TOT_PAID'],'TOT_CON' => $pitem['TOT_CON'] );
