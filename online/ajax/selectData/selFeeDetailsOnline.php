...
$query	=	" SELECT `CLASS_ID`, CONCAT(`Standard`, '-', `Section`) AS CLASS_SECTION, `Standard` AS STD, `Section` AS SEC, `NAME`, `FATHER_NAME`, `ADMISSION_ID`,`contact` FROM `v_studentlist` WHERE `ADMISSION_ID` = '".$adno."' and `Year_Id` = '".$yearId."'";
...
/*$sql_feehead = "SELECT * FROM tbl_bill where `status` = '1'  and Year_Id = '3'";
...
$arrayfee = array();
	...
	WHERE fee.Admission_No  IN (SELECT Admission_No FROM student_class_map WHERE Admission_No =  '".$arr['ADNO']."' AND Year_Id = '".$yearId."' ) AND feestatus.Admission_Id = '".$arr['ADNO']."' AND feestatus.Year_Id = '".$yearId."' order by feestatus.Fee_Headid ";
...
if( $cntFeeHistory > 0 ) {
	...
	while( $row = $exeFeeHistory->fetch_assoc() ) {
		...
		//$arrT2['TOT_PAID_AMD'] += $arrT2['TOT_HISTORY'];
	}
	$newArr	=	[];
