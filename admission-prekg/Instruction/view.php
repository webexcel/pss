...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_applied') {
    ...
    $query = "UPDATE application_prekg SET applied = '$applied' WHERE id = $id";
						...
						<div class="container">
							...
							<div class="row">   						
								<table  class="datatable table table-hover table-bordered">
									...
									count(CASE WHEN `fsubmit` = 'Y' THEN 1 END) as fsubmit,
									count(CASE WHEN `sel_list` = 2 THEN 1 END) as moved FROM `application_prekg` GROUP BY `applied`");
							...
							<div class="row">   						
								<table  class="datatable table table-hover table-bordered">
									...
									<tbody style="border-top:#585454;">	
										...
										$countrow = 1;
										$result = mysqli_query($dbconnect,"select * from application_prekg where status = '0'");
										while($row = mysqli_fetch_assoc($result))
										  {
											$id           = $row['id'];
											$fno          = $row['fno'];
