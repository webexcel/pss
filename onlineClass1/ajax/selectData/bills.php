...
if( isset($_GET['mode']) && $_GET['mode'] == 'newbill' ) {
	...
	FROM v_fees fee   LEFT OUTER JOIN 
	v_feestrans tran ON 
	fee.feeHeadId = tran.feeHeadId  AND fee.CLASS_ID = tran.CLASS_ID 
	...
	//$arr['SRC']	=	$dirPdfPath	;
	$json_response = json_encode($feeDetails);
	echo $json_response;
...
$yearId	=	$_SESSION['YEAR_ID'];
if( $section == "" ) {
	$query	=	" SELECT T2.`CLASS_ID`, T2.Standard AS STD, T2.Section AS SEC, T1.`NAME`, T1.`FATHER_NAME`, T1.`ADMISSION_ID` FROM student_info1 T1 LEFT JOIN tbl_class T2 ON T1.CLASS_ID = T2.CLASS_ID WHERE T1.ADMISSION_ID NOT IN (SELECT DISTINCT(ADMISSION_ID) FROM `fee_receipt` WHERE STATUS = 0 AND YEAR_ID = '".$yearId."') ORDER BY T1.CLASS_ID, T1.NAME ";
} else {
