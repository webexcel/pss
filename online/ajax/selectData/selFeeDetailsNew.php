...
if($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		...
		if($_SESSION['SESS_MEMBER_DBNAME'] == 'bedes') {
			$classId			=	$row['CLASS_ID'];
			if($classId >= 1 && $classId <= 4) {
				$arr['U_PDF']	=	$_SESSION['SESS_MEMBER_DBNAME'];
			} else {
				$arr['U_PDF']	=	$_SESSION['SESS_MEMBER_DBNAME']."1";
			}
		} else {
			$arr['U_PDF']	=	$_SESSION['SESS_MEMBER_DBNAME'];
		}
...
$sqlStuInfo	=	" SELECT * FROM `student_info1` WHERE `ADMISSION_ID` = '".$arr['ADNO']."' AND `Type` = 'Yes' ";
$exeStuInfo	=	$mysqli->query($sqlStuInfo);
$cntStuInfo	=	$exeStuInfo->num_rows;
...
if( $cntStuInfo > 0 ) {
	...
	WHERE Fee.Class_ID  IN (SELECT Class_ID FROM student_info1 WHERE Admission_Id =  '".$arr['ADNO']."' ) AND fee.feeheadId = 2 ";
} else {
