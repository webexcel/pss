...
if (empty($_POST['razorpay_payment_id']) === false)
{
	...
	/////////////////////////////
		$qry = "Select order_id  from `razorpay`  where order_id = '".$_POST['razorpay_order_id']."'";
		$result = $dbconnect->query($qry);
	...
	if($razorpaystatus == 'captured')
	{
		if ($success === true)
		{
			$sql = "update  `razorpay` set `end_time` = now() , `payment_id` ='".$razorpay_payment_id."' , `signature` = '".$_POST['razorpay_signature']."', `status`='COMPLETED' where order_id = '".$razorpayOrder_id."' ";
			...
			if ($dbconnect->query($sql) === TRUE) {
			  $html = "<div class='container'>
				<div id='login-box' class='col-md-12'>
				  ...
				  $html = "<td>Your payment was successful, yet somthing went wrong ".$dbconnect->error ."</p>
			...
			$sql = "update  `razorpay` set end_time = now() , payment_id ='".$_POST['razorpay_payment_id']."' , signature = '".$_POST['razorpay_signature']."', status='FAILED',remarks='".$error."' where order_id = '".$_POST['razorpay_order_id']."' ";
