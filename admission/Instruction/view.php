...
// Handle AJAX inline edit for Applied field
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_applied') {
    $id = intval($_POST['id']);
    $applied = mysqli_real_escape_string($dbconnect, trim($_POST['applied']));
    $query = "UPDATE application SET applied = '$applied' WHERE id = $id";
    echo mysqli_query($dbconnect, $query) ? 'OK' : 'ERROR';
    exit;
}
...
<style>
    ...
    .editable-applied:hover {
        background-color: #f0f8ff;
    }
    .editable-applied input {
        width: 100%;
        padding: 2px 5px;
						...
						<div class="container">
							...
							<div class="row">   						
								<table  class="datatable table table-hover table-bordered">
									...
									count(CASE WHEN `sel_list` = 2 THEN 1 END) as moved,
                                    count(CASE WHEN `sel_list` = 4 THEN 1 END) as Interview FROM `application` GROUP BY `applied`");
							...
							<div class="row">   						
								<table  class="datatable table table-hover table-bordered">
									...
									<tbody style="border-top:#585454;">	
										...
										$countrow = 1;
										$result = mysqli_query($dbconnect,"select * from application where status = '0'");
										while($row = mysqli_fetch_assoc($result))
										  {
											$id           = $row['id'];
											$fno          = $row['fno'];
										   ...
										   <tr>
												...
												<td> <?php echo $fno; ?> </td>
												<td class="editable-applied" data-id="<?php echo $id; ?>" data-value="<?php echo $group; ?>"> <?php echo $group; ?> </td>
												<td> <?php echo $name; ?> </td>
												<td> <?php echo $dob; ?> </td>
...
<script>
$(document).on('click', '.editable-applied', function () {
    var td = $(this);
    if (td.find('input').length > 0) return; // already editing

    var currentVal = td.data('value');
    var id = td.data('id');
    ...
    function saveValue() {
        ...
        if (newVal === '') newVal = currentVal; // revert if empty
        ...
        if (newVal !== currentVal) {
            $.ajax({
                url: 'view.php',
                type: 'POST',
                data: { action: 'update_applied', id: id, applied: newVal },
                success: function (res) {
                    if (res !== 'OK') {
                        alert('Failed to update');
                        td.html(currentVal);
                        td.data('value', currentVal);
                    }
                },
                error: function () {
    ...
    input.on('keydown', function (e) {
        ...
        if (e.key === 'Escape') { td.html(currentVal); }
    });
});
</script>
