...
if(!empty($data)) {
	...
	if($section != "" ) {
		$classId	=	$section;		
		$where	=	" AND T2.CLASS_ID = '".$classId."' ";
		$group = ",T2.CLASS_ID";
	}
	
	$query = "SELECT T3.FSID,  T1.ADMISSION_ID, T1.NAME, T1.FATHER_NAME, T2.Standard AS STD, T2.Section AS SEC, T3.Paid_Period AS LAST_PAID, sum(T3.Balance_Amount) AS TOT_BAL, T4.feehead AS FEE_HEAD, T4.feetype AS FEE_TYPE, T4.interval AS FEE_INTERVAL, T4.instalment AS FEE_INSTALMENT,sum(T4.feeamount) as TOT_AMT
		FROM student_info1 T1, tbl_class T2, feestatus T3, v_fees T4
		WHERE T1.ADMISSION_ID = T3.Admission_Id
		AND T4.CLASS_ID = T1.CLASS_ID
		AND T4.feeHeadId = T3.fee_headId
		AND T2.CLASS_ID = T1.CLASS_ID ".$where."
	...
	*/
		...
		AND T4.CLASS_ID = T1.CLASS_ID
		AND T4.feeHeadId = T3.fee_headId
		AND T2.CLASS_ID = T1.CLASS_ID group by T1.ADMISSION_ID
...
if($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) {
		...
		$paid_amount = "SELECT COALESCE(sum(fee_transanction.Amount),0) as TOT_PAID FROM `fee_receipt` join fee_transanction on (`fee_receipt`.`RECEIPT_ID` = fee_transanction.`RECEIPT_ID`) where fee_receipt.`ADMISSION_ID` ='".$row['ADMISSION_ID']."' and fee_receipt.status = '0'";
		...
		$narr['FEE_HEAD']	=	$row['FEE_HEAD'];
		$narr['FEE_TYPE']	=	$row['FEE_TYPE'];
		$narr['TOT_AMT']	=	$row['TOT_AMT'];	
		$narr['TOT_BAL']	=	$row['TOT_BAL'];
		$narr['TOT_PAID']	=	$row1['TOT_PAID'];
		$narr['FEE_INTERVAL']	=	$row['FEE_INTERVAL'];
		$narr['FEE_INSTALMENT']	=	$row['FEE_INSTALMENT'];
		
		$type		=	$row['FEE_TYPE'];
		$interval	=	$row['FEE_INTERVAL'];
		$preBalance	=	$row['TOT_BAL'];
		$instalment	=	$row['FEE_INSTALMENT'];
		$lPaid		=	$row['LAST_PAID'];

		$narr['LAST_PAID']	=	$lPaid;
		$asOfNowBalance		=	calBalance($type, $lPaid, $interval, $instalment, $preBalance);
		$narr['BALANCE']	=	$asOfNowBalance;
