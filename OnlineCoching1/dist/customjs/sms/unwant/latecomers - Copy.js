	...
	fetch.controller('dbCtrl', function($window,$location,$scope, $filter, $http, $timeout) {
		...
		$scope.saveStudent = function(saveValues) {
			...
			if ($scope.myForm.$valid) {			
				$http({
					method: "post",
					url: "ajax/sms/inslatecomers.php",
		...
		$scope.getlatecomerslist = function() {
			var request = $http({
				method: "post",
				url: "ajax/sms/getlatecomerslist.php",
		...
		$scope.sendlate = function() {
			console.log($scope.dStudent1);							
			$http({
				method: "post",
				url: "ajax/sms/sendLatecomers.php",
				data: {
					uStudent: $scope.dStudent1
					},
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			}).success(function(response) {
