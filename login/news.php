...
if(isset($_POST['btn_action'])){
   extract($_POST);		
	if(isset($_POST['btn_action']))
	{
		$query= "insert into news (`past_event`, `latest_event`, `eventDate`) values ('".$pevent."','".$levent."','".$eventDate."')";
    ...
    if(isset($_POST['btn_action']))
	    {
    	$query= "UPDATE `news` SET `past_event`= past_event,`latest_event` = latest_event WHERE `nid`= nid";
			if(mysqli_query($dbconnect,$query))
			{
				$data['code']=1;
				$data['msg']="Success";
...
if(isset($_POST['deletenews'])){
	$query= "delete from news where nid = ".$_POST['hiddencat'];
	if(mysqli_query($dbconnect,$query)){
		$data['code']=1;
...
$rs=mysqli_query($dbconnect,"SELECT * FROM  `news` ORDER BY nid DESC") or die(mysql_error());


$newsid = $_GET['newsid'];
$rq=mysqli_query($dbconnect,"SELECT * FROM  `news` where nid = '".$newsid."' ") or die(mysql_error());
?>
				...
				<!-- Contant -->
					<div class="col-md-12">
						...
						<div class="row">						
							<div class="form-group">
								<div class="col-sm-4">
									<textarea class="form-control" id="pevent" name="pevent" rows="5" placeholder="Enter Past events"></textarea>
								</div>
							</div>
						</div>
						<br>
						<div class="row">						
							<div class="form-group">
								<div class="col-sm-4">
									<textarea class="form-control" id="levent" name="levent" rows="5" placeholder="Enter Latest events"></textarea>
								</div>
							</div>
						</div>
						<br>
						...
						<table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th width="40%" class="text-left">Past Events</th>
									<th width="40%" class="text-left">Latest Events</th>
									<th width="10%" class="text-left">Events Date</th>
                              ...
                              <?php
                               ...
                               ?>          
							    <tr>
									...
									<td><?php echo $row['past_event']; ?></td>
									<td><?php echo $row['latest_event']; ?></td>
									<td><?php echo $row['eventDate']; ?></td>
									<td class="txt-center">
									   <a href="newsedit.php?newsid=<?php echo $row['nid']; ?>">edit</a>
									</td>
									<td class="txt-center">
									   <a class="catgoryclick" cid="<?php echo $row['nid'] ?>">delete</a>
									</td>
								</tr>
