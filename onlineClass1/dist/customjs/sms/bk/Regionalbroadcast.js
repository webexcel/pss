var app = angular.module('ngApp', ['ui.bootstrap', "ngSanitize", "ngCsv"])
    ...
    app.controller('studentsCtrl', ['$http', '$scope', '$timeout',  function ($http, $scope, $timeout) {
		...
		$scope.sendBroadcastRegional = function(aStudent) {			
			console.log($scope.aStudent);								
			$http({
				method: "post",
				url: "ajax/sms/sendBroadcastRegional.php",
				data: { aStudent: $scope.aStudent },
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			}).success(function(response) {					
				$scope.response	= response;	
				$scope.getBroadcastRegional();			
		...
		$scope.getBroadcastRegional = function() {
			var request = $http({
				method: "POST",
				url: "ajax/sms/getBroadcastRegional.php",
