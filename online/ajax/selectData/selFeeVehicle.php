...
if($result->num_rows > 0) {
	...
	$stage		= "SELECT * FROM veh_routes where `status` = '1'  ";
	$exestage	=	$mysqli->query($stage);
	$arrayfee 	= array();
	while( $row = $exestage->fetch_assoc() ) {
		$arraystage['stage'][] = $row;
	}
...
if( $cntFeeDetails > 0 ) {
	...
	while( $row = $exeFeeDetails->fetch_assoc() ) {
		...
		//$arrFeeDetails['PAID_FEE'] 		= $row['PAID_FEE'] + $arrFeeDetails['CONCESSION_AMOUNT'];
