			...
			<div class="main-content-inner">
				...
				<div class="page-content">
					<div class="page-header">
						<h1>Online Coaching Class Fees Collection</h1>
					</div>
					<!-- /.page-header -->
					<div class="row">
						<div class="col-md-12">
					...
					<div class="row" ng-show="user.ADNO != NULL">
						...
						<div class="row">
							...
							<div class="col-md-6" style="padding-top:17px;">
									...
									<tr>
									<td colspan="2">Total Coaching Fee</td>
										<td>
											<span style="color:red">RS.{{ user.FEE_TOTAL }}</span>
										</td>
									</tr>
									<tr ng-show="user.PAID > 0">
									<td colspan="2">Already Paid</td>
										<td>
											<span>RS.{{ user.PAID }}</span>
										</td>
									</tr>
									<tr>
									<td colspan="2">Amount to Pay Now</td>
										<td>
											<span style="color:red"><b>RS.{{ user.amount }}</b></span>
										</td>
									</tr>
									<tr ng-show="user.amount == 0 && user.ADNO != null">
										<td colspan="3" class="text-center" style="color:green;"><b>Fees Fully Paid</b></td>
									</tr>
									<tr ng-show="user.amount > 0 && user1.pay_id == null">
									<td colspan="2">Sport Type</td>
										<td>
											<select class="form-control" name="game" id="game" ng-model="user.game" ng-required="user1.pay_id == null">
												<option value="">Select</option>
							...
							<div class="col-md-6" style="padding-top:17px;" ng-show="history.length > 0">
							<!--<div class="col-md-6" style="padding-top:17px;" ng-repeat="(key1, val1) in user2">	-->							
								<div class="table-header table-header profile-user-info width mb-5"><b>FEES PAID HISTORY</b></div>                
									...
									<tr ng-repeat="h in history">
										...
										<td><a href='bills/pssenior.php?r={{h.pay_id}}&adno={{user.ADNO}}&amount={{h.amount}}&name={{user.NAME}}&game={{h.game}}&date={{h.date}}&classsec={{user.SECTION}}&yearid=6' target='_blank' class="btn btn-minier btn-success no-border"><i class="fa fa-print" aria-hidden="true"></i>&nbsp;Print</a></td>
									</tr>
									<tr>
										<td colspan='2'>Total</td>
										<td ><b>{{ history | total:'amount' }}</b></td>
										<td></td>
									</tr>
									</table>
							</div>	
	...
	fetch.controller('dbCtrl', function($window,$location,$scope,$filter, $http, $timeout, $modal) {
		...
		$scope.payFees	=	function(user) {						
			...
			if ($scope.myForm.$valid) {
				...
				// top-up payers (already paid 3500) keep their existing sport
				var game		=	user.game ? user.game : ($scope.user1 ? $scope.user1.game : "");
				var amount		=	user.amount;	// balance to pay now (5000 new / 1500 top-up)
				if (!amount || amount <= 0) { alert("Nothing to pay. Fees already fully paid."); return false; }
				stuFeeDetails1.push({  "ADNO":fadno,"name":name,"classsec":classsec,"game":game,"amount":amount});
		...
		$scope.GetAdno = function() {
			...
			request.success(function (data1) {
				...
				// TOT_HISTORY is now an array of all payment rows (first payment + top-ups)
				$scope.history = angular.isArray(data1.TOT_HISTORY) ? data1.TOT_HISTORY : [];
				// keep a scalar summary (latest payment) for the "already paid / existing game" gating
				$scope.user1 = $scope.history.length ? $scope.history[$scope.history.length - 1] : {};
				console.log("welcome",$scope.history);
