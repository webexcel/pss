...
$db = getDbInstance();
$select = array('sn', 'title', 'created_at', 'updated_at');


$chunk_size = 100;
$offset = 0;

$data = $db->withTotalCount()->get('contents');
$total_count = $db->totalCount;

$handle = fopen('php://memory', 'w');

fputcsv($handle,$select);
$filename = 'export_contents.csv';
...
for ($i=0; $i<$num_queries; $i++){
    ...
    $rows = $db->get('contents',Array($offset,$chunk_size), $select);
