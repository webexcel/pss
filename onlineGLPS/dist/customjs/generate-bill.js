var app = angular.module('ngApp', ['ui.bootstrap', "ngSanitize", "ngCsv"])
    ...
    app.controller('receiptsCtrl', ['$window','$location','$http', '$scope', '$timeout',  function ($window,$location,$http, $scope, $timeout) {
		...
		$scope.getbills = function() {
			$http.get('ajax/selectData/getbillbookid.php')
			.success(function(response){
				//console.log(response);
		...
		$scope.getStudents = function() {
			var request = $http({
				method: "POST",
				url: "ajax/selectData/get-generate-bill.php",
				//data: $scope.data.section,
				data: { section: $scope.section , bills: $scope.billname },
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			});
		...
		$scope.getpdfbulk = function() {
			var request = $http({
				method: "POST",
				url: "bills/demoschchallan.php",
				data: { section: $scope.section , bills: $scope.billname },
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			});
			request.success(function (response) {
		...
		$scope.newBill = function(sDet) {
			console.log(sDet);
			var sAdno = sDet.ADMISSION_ID

			$http({
				method: "post",
				url: "ajax/selectData/bills.php?mode=newbill",
				data: {
					sAdno: sAdno
				},
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
		...
		$scope.generateBill = function(sDetail) {
			$http({
				method: "post",
				url: "ajax/selectData/get-generate-pdfbill.php",
				data: {
					sDetail: sDetail
				},
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
