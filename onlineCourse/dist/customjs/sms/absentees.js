	...
	fetch.controller('dbCtrl', function($window,$location,$scope, $filter, $http, $timeout) {
		...
		$scope.myFunc = function(YearId) {	
			...
			$http({
				...
				$window.location.href = "latecomers.php"; //You should have http here.
				console.log(response);
		...
		$scope.saveStudent = function(saveValues) {
			...
			if ($scope.myForm.$valid) {			
				$http({
					method: "post",
					url: "ajax/sms/absentees/insAbsentees.php",
					data: { adno : $scope.reguser },
					headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
				}).success(function(response) {
					console.log(response);	
		...
		$scope.getAbsenteeslist = function() {
			var request = $http({
				method: "post",
				url: "ajax/sms/absentees/getAbsenteeslist.php",
		...
		$scope.sendAll = function() {
			...
			$http({
				method: "post",
				url: "ajax/sms/absentees/sendAllAbsentees.php",
				data: { uStud: $scope.ID },
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
			}).success(function(response) {
		...
		$scope.delAbsentees = function() {
			console.log($scope.dStudent);							
			$http({
				method: "post",
				url: "ajax/sms/absentees/delAbsentees.php",
				data: {
		...
		$scope.sendAbsentees = function() {
			console.log($scope.dStud);							
			$http({
				method: "post",
				url: "ajax/sms/absentees/sendAbsentees.php",
				data: {
					uStud: $scope.dStud
					},
				headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
